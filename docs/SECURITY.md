...
- Plugin `PreToolUse` Hook 是授权内核（ADR-0011）的宿主侧客户端与即时提示：初始化项目内的 `apply_patch` 目标路径与 Shell 重定向目标经 `codex-os authorize-hook` 由统一授权内核裁决（运行模式下命中 protected/治理规则路径即拒绝），本地正则列表仅在 Runtime 不可达时作为显式降级兜底。Hook 是尽力而为的强制点——宿主可禁用 Hook，且不承诺解释变量拼接、别名、splatting 或间接脚本；最终控制仍由 Runtime 的结构化 argv allowlist、风险分级、路径校验、审批、OCI 隔离与事前 Git 证据 fail-closed 校验共同承担（三层强制模型）。项目 `.codex/hooks.json` 有意不复制插件规则；插件未启用或 Hook 未经信任时，Runtime 仍必须 fail closed。
