...
<!-- codex-os-document: {"schema_version":"1.2","document_version":"0.2.1","status":"approved","owner":"architect","requirement_refs":["REQ-1.6.2","REPO-001","GATE-001","AGENT-001","HANDOFF-001","WORKTREE-001","RELEASE-001","EXEC-001","MEMORY-001","ROUTING-001"]} -->
...
Windows 上的绝对项目根与 Worktree 路径统一以 `Path.resolve().as_posix()` 形式保存到 SQLite，并通过 UTF-8 JSON/stdio 传输；Runtime 拒绝向状态库写入包含 Unicode replacement character `U+FFFD` 的路径。`codex-os doctor --project-root ...` 只读扫描路径字段，发现历史损坏记录时返回 `PATH_ENCODING_CORRUPT` 及 table/record/field，不猜测或自动改写原路径。
...
| 路径/执行 | `PATH_DENIED`、`PATH_ESCAPE`、`PATH_ENCODING_CORRUPT`、`COMMAND_DENIED`、`APPROVAL_REQUIRED`、`SANDBOX_UNAVAILABLE`、`SANDBOX_IMAGE_UNAVAILABLE`、`EXECUTION_FAILED`、`EXECUTION_DIRTY` |
