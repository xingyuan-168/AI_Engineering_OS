...
2. 创建数据库备份并校验可读性。
3. 校验所有已应用迁移 checksum 未漂移，在独立事务中按顺序执行未应用迁移至 `0008`。
...
`0008` 对被引用的 Host Operation 表使用 create-copy-drop-rename 重建，禁止先改名旧表
导致 Release/Audit 外键重定向。只有该受信迁移在事务开始前临时关闭 FK enforcement，
提交前必须执行 `foreign_key_check`，成功或失败都恢复 enforcement。
历史 `0006 -> 0007` intent 引导仍从不可变 0007 SQL 获取定义，不随最新版本漂移。
真实本机迁移必须先暂停 AI-OS 写入、备份并验证；本轮开发测试不迁移活动数据库。
